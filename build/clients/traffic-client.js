...
function reportTraffic(report) {
    try {
        ...
        var icaoAddress_1 = report[icaoAddressKey].toString();
        // Create the entry if it is not already there.
        if (trafficCache[icaoAddress_1] == null) {
            trafficCache[icaoAddress_1] = report;
            //console.debug(`TRAFFIC:${Date.now().toLocaleString()}: Adding ${icaoAddress}`);
        }
        else {
            // Now go and perform the painful merge
            Object.keys(report).forEach(function (key) {
                trafficCache[icaoAddress_1][key] = report[key];
            });
        }
        lastWebsocketReportTime = Date.now();
        trafficCache[icaoAddress_1][secondsSinceLastReportKey] = lastWebsocketReportTime;
    }
    catch (e) {
...
//# sourceMappingURL=traffic-client.js.map